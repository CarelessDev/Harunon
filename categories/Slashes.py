...
class Slash(commands.Cog):
    ...
    @cog_ext.cog_slash(name="blep",description="people", guild_ids=guild_ids,
    ...
    async def _ase(self, ctx:SlashContext, start:str):
       ...
       if not bool(int(start)):
            embed = discord.Embed(title="imagine won't even try to answer chi questions", color=0x66ff99)
            ...
            for question, answers in questions:
                ...
                select = create_select(
                            ...
                            placeholder="Choose your answer",  # the placeholder text to show when no options have been chosen
                            min_values=1,  # the minimum number of options a user must select
                            max_values=1,  # the maximum number of options a user can select
                        )
                embed = discord.Embed(title=question, color=0x66ff99)
                await ctx.send(embed=embed, components=[create_actionrow(select), action_row])  # like action row with buttons but without * in front of the variable
                ...
                for i in range(2):
                    ...
                    if not button_ctx.selected_options:
                        if button_ctx.component_id == 'q':
                            ...
                            embed = discord.Embed(title="smh imagine skipping", color=0x66ff99)
                            embed.set_author(name=button_ctx.author.name, icon_url=button_ctx.author.avatar_url)
                            await button_ctx.send(embed=embed)
                            break
                        
                    elif list(button_ctx.selected_options) == ['1']:
                        answered = [x for x in button_ctx.component['options'] if x['value'] == '1'][0]['label']
              
                        embed = discord.Embed(title=f"{button_ctx.author.nick} is right!!!, the answer is {answered}", color=0x66ff99)
                        embed.set_author(name=button_ctx.author.name, icon_url=button_ctx.author.avatar_url)
                        await button_ctx.send(embed=embed)
                        score += 1
                        break
                    else:
                        answered = [x for x in button_ctx.component['options'] if list(x['value']) == button_ctx.selected_options][0]['label']
                        #print(button_ctx.selected_options)
                        if i < 1:
                            embed = discord.Embed(title=f"Ha {button_ctx.author.nick} suck!, it is not {answered} try again", color=0xff6666)
                            embed.set_author(name=button_ctx.author.name, icon_url=button_ctx.author.avatar_url)
                            await button_ctx.send(embed=embed)
                        else:
                            embed = discord.Embed(title=f"What a foking idiot {button_ctx.author.nick},it is also not {answered} u cant even get it right the second time{'.' if amount == len(questions) else ', anyway next.'}", color=0xf1c40f)
                            embed.set_author(name=button_ctx.author.name, icon_url=button_ctx.author.avatar_url)
                            await button_ctx.send(embed=embed)
                if button_ctx.component_id=="q":
                    break
            embed = discord.Embed(title=f"welp that the end, the score: {score}/{amount}", color=0x66ff99)
            await ctx.send(embed=embed)
